#define johannesburgo2sydney_cxx
#include "johannesburgo2sydney.h"
#include <TH2.h>
#include <TGraph.h>
#include <TStyle.h>
#include <TCanvas.h>
...
Double_t calculaE( Double_t Ngamma, Double_t lambdaprom );
Double_t MeV2Joule(Double_t EnergyInMeV);

void johannesburgo2sydney::Loop()
{
...
//      NCherenkov_electron_uvA_Array[ jentry ] = nElectronPositron * calculaN(momentum,m_e,400E-9,315E-9);
   ...
   Double_t totaluva = totalNCherenkov_electron_uvA + totalNCherenkov_positron_uvA + totalNCherenkov_muminus_uvA + totalNCherenkov_muplus_uvA + totalNCherenkov_p_uvA +  totalNCherenkov_pbar_uvA;
   Double_t totaluvb = totalNCherenkov_electron_uvB + totalNCherenkov_positron_uvB + totalNCherenkov_muminus_uvB + totalNCherenkov_muplus_uvB + totalNCherenkov_p_uvB +  totalNCherenkov_pbar_uvB;
   Double_t totaluvc = totalNCherenkov_electron_uvC + totalNCherenkov_positron_uvC + totalNCherenkov_muminus_uvC + totalNCherenkov_muplus_uvC + totalNCherenkov_p_uvC +  totalNCherenkov_pbar_uvC;
   cout << "Total N Cherenkov UV-A: " << totaluva << endl;
   cout << "Total N Cherenkov UV-B: " << totaluvb << endl;
   cout << "Total N Cherenkov UV-C: " << totaluvc << endl;
   //
   Double_t E_uva = calculaE(totaluva, 357.5);
   Double_t E_uvb = calculaE(totaluvb, 297.5);
   Double_t E_uvc = calculaE(totaluvc, 190);
   cout << "Energy N Cherenkov UV-A: " << E_uva << " MeV" << endl;
   cout << "Energy N Cherenkov UV-B: " << E_uvb << " MeV" << endl;
   cout << "Energy N Cherenkov UV-C: " << E_uvc << " MeV" << endl;
   //
   cout << "UV-A E/m: " << MeV2Joule(E_uva)/100.0 << " J/kg" << endl;
   cout << "UV-B E/m: " << MeV2Joule(E_uvb)/100.0 << " J/kg" << endl;
   cout << "UV-C E/m: " << MeV2Joule(E_uvc)/100.0 << " J/kg" << endl;
  ...
  c1->cd();
  c1->SetSelected(c1);
  c1->SaveAs("johannesburg2sydney_electronUV.png");
  c1->Close();
  //------------------------------------------------------------------------------------------------
  ...
  c2->cd();
  c2->SetSelected(c2);
  c2->SaveAs("johannesburg2sydney_positronUV.png");
  c2->Close();
  //------------------------------------------------------------------------------------------------
  ...
  c3->cd();
  c3->SetSelected(c3);
  c3->SaveAs("johannesburg2sydney_muminusUV.png");
  c3->Close();
  //------------------------------------------------------------------------------------------------
  ...
  c4->cd();
  c4->SetSelected(c4);
  c4->SaveAs("johannesburg2sydney_muplusUV.png");
  c4->Close();
  //------------------------------------------------------------------------------------------------
  ...
  c5->cd();
  c5->SetSelected(c5);
  c5->SaveAs("johannesburg2sydney_pUV.png");
  c5->Close();
  //------------------------------------------------------------------------------------------------
  ...
  c6->cd();
  c6->SetSelected(c6);
  c6->SaveAs("johannesburg2sydney_pbarUV.png");
  c6->Close();
...
Double_t calculaE( Double_t Ngamma, Double_t lambdaprom ){
  Double_t nwater = 1.33;
  Double_t hbarc = 0.0001973269718; // MeV nm

  return 2*TMath::Pi()*Ngamma*hbarc/(nwater*lambdaprom);
}



Double_t MeV2Joule(Double_t EnergyInMeV){
  return 1.602176565E-13*EnergyInMeV;
}
